...
## Generate MAR and visualization of mv in the dataset
...
tmp<-delete_MAR_1_to_x(df_no_miss, p = c(0.2), cols_mis = c("displacement"), cols_ctrl =c("horsepower"), x =100)
...
```{r}
fit_complex_mean<-lm(I(log(mpg))~weight+horsepower + cut_model_year, data = complex_ds_mean)
fit_complex_del<-lm(I(log(mpg))~weight+horsepower + cut_model_year, data = complex_ds_del)
```
...
```{r}
# Extract coefficients and confidence intervals
fit_models <- list(mean = fit_complex_mean, list = fit_complex_del, or = fit_multi1)
...
df <- data.frame(predicted = predict(fit_complex_mean))

# Creazione del grafico a dispersione
ggplot(df, aes(x = predicted, y = resid(fit_complex_mean))) +
    geom_point(color = nord_colors[3]) +
    geom_smooth(method = "lm", se = FALSE, color = nord_colors[2], formula = y ~ x, linewidth = 0.75, linetype = 2) +
    geom_smooth(method = "loess", se = FALSE, color = nord_colors[1], formula = y ~ x) +
    xlab("Valori fittati di mpg") +
    ylab("Residui") +
    ggtitle("Residui Vs Valori fittati")+
    theme_bw()

# Creazione del data frame dei residui standardizzati
fitted_values <- predict(fit_complex_mean)
standardized_residuals <- rstandard(fit_complex_mean)

# Creazione del data frame dei residui standardizzati
residuals_df <- data.frame(Standardized_Residuals = standardized_residuals)

# Have to merge this needs fixk 
scale_location_df <- data.frame(
  Fitted_Values = fitted_values,
  Standardized_Residuals = sqrt(abs(standardized_residuals))
)

ggplot(scale_location_df, aes(x = Fitted_Values, y = Standardized_Residuals)) +
    geom_point(color = nord_colors[3]) +
    geom_smooth(method = "lm", se = FALSE, color = nord_colors[2], formula = y ~ x, linewidth = 0.75, linetype = 2) + 
    geom_smooth(method = "loess", se = FALSE, color = nord_colors[1], formula = y ~ x) +
    xlab("Fitted Values") +
...
# Creazione del QQ plot dei residui standardizzati
ggplot(residuals_df, aes(sample = Standardized_Residuals)) +
    stat_qq(color = nord_colors[3]) +
    stat_qq_line() +
    labs(title = "QQ Plot ", x="Quantili teorici", y="Residui standardizzati") +
    theme_bw()

# Grafico dei residui standardizzati

# Grafico dei residui
ggplot(complex_ds_mean, aes(x = mpg, y = resid(fit_complex_mean))) +
    geom_point(color = nord_colors[3]) +
    geom_smooth(method = "lm", se = FALSE, color = nord_colors[2], formula = y ~ x, linewidth = 0.75, linetype = 2) +
    geom_smooth(method = "loess", se = FALSE, color = nord_colors[1], formula = y ~ x) +
    labs(title="Residui Vs variabile esplicativa", x = "mpg", y = "Residui") +
    theme_bw()
```
...
## guardare dens var ricostruite? prob no
## scatterplot con var ricostruite? prob no