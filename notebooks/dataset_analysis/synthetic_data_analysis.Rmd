...
Missing values are inserted randomly across the dataset, either using Bernoulli trials or random selection methods. Common configurations include univariate (one feature) or multivariate (multiple features) generation, with random positions selected for deletion.
...
sum(is.na(data_mcar)) / nrow(data_mcar)  # Should be approximately 10%
sum(is.na(data_mar)) / nrow(data_mar) # Should be approximately 10%
```
