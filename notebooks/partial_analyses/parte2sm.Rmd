...
create_plot <- function(data, x_var, x_label, title, color) {
  ggplot(data = data, mapping = aes(!!sym(x_var), mpg)) +
...
create_plot <- function(data, x_var, x_label, title, color) {
  ggplot(data = data, mapping = aes(!!sym(x_var), horsepower)) +
    geom_point(color = color) +
    xlab(x_label) +
    ylab("Mpg") +
    theme_bw() +
...
variables <- list(
  list(var = "displacement", label = "Cilindrata", title = "Scatterplot Cilindrata-Mpg", color = nord_colors[1]),
  list(var = "mpg", label = "Cavalli-potenza", title = "Scatterplot Cavalli-Mpg", color = nord_colors[2]),
  list(var = "weight", label = "Peso", title = "Scatterplot Peso-Mpg", color = nord_colors[3]),
  list(var = "acceleration", label = "Accelerazione", title = "Scatterplot Accelerazione-Mpg", color = nord_colors[4])
)
...
create_plot <- function(data, x_var, x_label, title, color) {
  ggplot(data = data, mapping = aes(!!sym(x_var), acceleration)) +
    geom_point(color = color) +
    xlab(x_label) +
    ylab("Mpg") +
    theme_bw() +
...
variables <- list(
  ...
  list(var = "mpg", label = "Cavalli-potenza", title = "Scatterplot Cavalli-Mpg", color = nord_colors[2]),
  list(var = "weight", label = "Peso", title = "Scatterplot Peso-Mpg", color = nord_colors[3]),
  list(var = "horsepower", label = "Accelerazione", title = "Scatterplot Accelerazione-Mpg", color = nord_colors[4])
)
...
mean_model<-lm(I(log(mpg))~weight+horsepower + cut_model_year, data = imp_ds_mean)
median_model<-lm(I(log(mpg))~weight+horsepower + cut_model_year, data = imp_ds_median)
...
```{r imp rf reg (gam), echo=FALSE}
rf_mice<-mice(missingMar, method = "rf", ntree = 200, mtry = 3, maxit = 5, m = 5, seed = 42)
reg_mice<-mice(missingMar, method = "norm")
```


```{r fit rf reg (gam), echo=FALSE}
#NON correttissimo fare così
imp_ds_rf_mice <- complete(rf_mice, action = 1)
imp_ds_reg_mice <- complete(reg_mice, action = 1)
rf_fit_WRONG<-lm(I(log(mpg))~weight+horsepower + cut_model_year, data = imp_ds_rf_mice)
reg_fit_WRONG<-lm(I(log(mpg))~weight+horsepower + cut_model_year, data = imp_ds_reg_mice)


#modo giusto
rf_model <- with(rf_mice, lm(I(log(mpg))~weight+horsepower + cut_model_year ))  
pooled_rf <- pool(rf_model)
summary(pooled_rf)
...
predictions1 <- predict(rf_fit_WRONG, newdata = test_data)
predictions2 <- predict(reg_fit_WRONG, newdata = test_data)
predictions3 <- predict(mean_model, newdata = test_data)
predictions4 <- predict(median_model, newdata = test_data)

predictions5 <- predict(fit_multi1, newdata = test_data)

# Fit the model using with()
rf_model <- with(rf_mice, lm(I(log(mpg)) ~ weight + horsepower + cut_model_year))

# Extract the imputed datasets
imputed_datasets <- complete(rf_mice, "all")

# Generate predictions for each imputed dataset
predictions_list <- lapply(imputed_datasets, function(data) {
  model <- lm(I(log(mpg)) ~ weight + horsepower + cut_model_year, data = data)
...
predictions6 <- predict(fit_list, newdata = test_data)



#Evaluate using RMSE
rmse_value <- RMSE(predictions1, test_data$mpg)
cat("Root Mean Squared Error (RMSE):", rmse_value, "\n")
rmse_value <- RMSE(predictions2, test_data$mpg)
cat("Root Mean Squared Error (RMSE):", rmse_value, "\n")
rmse_value <- RMSE(predictions3, test_data$mpg)
cat("Root Mean Squared Error (RMSE):", rmse_value, "\n")
rmse_value <- RMSE(predictions4, test_data$mpg)
cat("Root Mean Squared Error (RMSE):", rmse_value, "\n")

rmse_value <- RMSE(predictions5, test_data$mpg)
cat("Root Mean Squared Error (RMSE):", rmse_value, "\n")
...
rmse_value <- RMSE(predictions6, test_data$mpg)
cat("Root Mean Squared Error (RMSE):", rmse_value, "\n")
...
*split tt+, 1 sola cov con mv+, trovare buon modello+, fare rmse+, fare gam, fare deletion*
