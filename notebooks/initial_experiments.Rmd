...
```{r}
syntethic_data <- synthetic_dataset_gen(n_samples = 1000, n_covariates = 5, correlation = "linear", target_type = "linear", noise_level = 0.5)

cor_matrix <- cor(syntethic_data)
...
data_continuous <- syntethic_data[, sapply(data, is.numeric)]
...
data_mcar <- introduce_mcar(syntethic_data, prop_missing = 0.1, missing_cols = "X3")
data_mar <- introduce_mar(syntethic_data, prop_missing = 0.1, predictor_cols = c("X1", "X2"), target_cols = c("X3"))
