...
compare_imputed_to_original <- function(original_data, imputed_data) {
  if (is.data.frame(original_data) && is.data.frame(imputed_data)) {
    if (!all(names(original_data) == names(imputed_data))) {
      stop("Original and imputed data frames must have the same structure.")
    }
    differences <- sapply(names(original_data), function(col) {
      orig_col <- original_data[[col]]
      imp_col <- imputed_data[[col]]
      if (is.numeric(orig_col) && is.numeric(imp_col)) {
        mean(abs(orig_col - imp_col), na.rm = TRUE)
      } else {
        NA
      }
    })
    return(mean(differences, na.rm = TRUE)) # Average mean absolute difference across columns
  } else if (is.numeric(original_data) && is.numeric(imputed_data)) {
    return(mean(abs(original_data - imputed_data), na.rm = TRUE))
  } else {
    stop("Inputs must be numeric or data frames with numeric columns.")
  }
}
```
...
listwise_rmse <- compute_rmse(test_listwise$target, listwise_predictions)
...
listwise_mar_rmse <- compute_rmse(test_listwise_mar$target, listwise_mar_predictions)
...
pairwise_rmse <- compute_rmse(test_pairwise$target, pairwise_predictions)
...
pairwise_mar_rmse <- compute_rmse(test_pairwise_mar$target, pairwise_mar_predictions)
...
mean_rmse <- compute_rmse(test_mean$target, mean_predictions)

mean_model_mar <- lm(target ~ ., data = train_mean_mar)
mean_mar_predictions <- predict(mean_model_mar, test_mean_mar)
mean_mar_rmse <- compute_rmse(test_mean_mar$target, mean_mar_predictions)
...
median_rmse <- compute_rmse(test_median$target, median_predictions)

median_model_mar <- lm(target ~ ., data = train_median_mar)
median_mar_predictions <- predict(median_model_mar, test_median_mar)
median_mar_rmse <- compute_rmse(test_median_mar$target, median_mar_predictions)
...
regression_rmse <- compute_rmse(test_regression$target, regression_predictions)
...
regression_mar_rmse <- compute_rmse(test_regression_mar$target, regression_mar_predictions)
...
hotdeck_rmse <- compute_rmse(test_hotdeck$target, hotdeck_predictions)

hotdeck_model_mar <- lm(target ~ ., data = train_hotdeck_mar)
hotdeck_mar_predictions <- predict(hotdeck_model_mar, test_hotdeck_mar)
hotdeck_mar_rmse <- compute_rmse(test_hotdeck_mar$target, hotdeck_mar_predictions)
...
em_rmse <- compute_rmse(test_em$target, em_predictions)

em_model_mar <- lm(target ~ ., data = train_em_mar)
em_mar_predictions <- predict(em_model_mar, test_em_mar)
em_mar_rmse <- compute_rmse(test_em_mar$target, em_mar_predictions)
...
results <- data.frame(
  ...
  RMSE = c(baseline_rmse, listwise_rmse, pairwise_rmse, mean_rmse, median_rmse, regression_rmse, hotdeck_rmse, em_rmse),
  Mean_Absolute_Difference = c(NA, NA, NA, mean_diff, median_diff, regression_diff, hotdeck_diff, em_diff)
)
...
results_long <- melt(results, id.vars = "Method", variable.name = "Metric", value.name = "Value")

# Remove NA values
results_long <- na.omit(results_long)

# Define Nord colors
nord_colors <- c(
  "RMSE" = "#A3BE8C",  # Nord Green
  "Mean_Absolute_Difference" = "#bf616a"  # Nord Red
)

# Create combined plot
ggplot(results_long, aes(x = Method, y = Value, fill = Metric)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = nord_colors) +
  labs(
