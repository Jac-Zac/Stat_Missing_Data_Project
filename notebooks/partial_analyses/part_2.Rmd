...
```{r}
residuals_vs_leverage_plot <- function(model) {
  
  # 1) Compute diagnostics used by plot.lm(which = 5)
  h      <- hatvalues(model)      # leverages
  rstud  <- rstudent(model)       # studentized residuals
  p      <- length(coef(model))   # number of parameters
  n      <- length(rstud)
  
  # 2) Build the base data frame for the main scatter
  df <- data.frame(
    Leverage   = h,
    RStudent   = rstud
  )

  # 3) Build Cook’s distance reference lines at 0.5 and 1.0
  #    Cook’s D = 0.5 or 1 =>  rstudent^2 = D * p * (1 - h)/h
  #    => rstudent = ± sqrt(D * p * (1 - h)/h)
  cooks_vals <- c(0.5, 1.0)
  
  # We'll only plot the curve for h > a small cutoff to avoid exploding near h=0
  h_seq <- seq(min(h[h > 1e-5]), max(h), length.out = 200)
  
  make_cooks_curve <- function(Dval) {
    # Cook’s distance => ± sqrt(...)
  ...
  ggplot(df, aes(x = Leverage, y = RStudent)) +
    # Base scatter: observation points
    geom_point(shape = 1, color = nord_colors[6]) +
    # The red lowess line (like base R)
    geom_smooth(method = "loess", se = FALSE, color = nord_colors[2]) +
...
#SE VOGLIAMO METTERE I PUNTI VUOTI SCRIVERE geom_point(shape = 1, color = nord_colors[6]) +

#function to obtain plot.lm() using ggplot2
my_diagnostic_plots <- function(model, data) {
  library(ggplot2)
  library(patchwork)
  ...
  p1 <- ggplot(df1, aes(x = Fitted, y = Residual)) +
    geom_point(shape = 1,color = nord_colors[6]) +
    geom_smooth(method = "lm", se = FALSE, color = nord_colors[2],
                formula = y ~ x, linewidth = 0.75, linetype = 2) +
    geom_smooth(method = "loess", se = FALSE, color = nord_colors[1],
                formula = y ~ x) +
    labs(
  ...
  # (c) Normal Q-Q plot of standardized residuals
  p3 <- ggplot(df3, aes(sample = Standardized_Residuals)) +
    stat_qq(shape = 1,color = nord_colors[6]) +
    stat_qq_line(color = nord_colors[2]) +
    labs(
      title = "Normal Q-Q",
  ...
  # (d) Cook's distance (4th plot in plot.lm)
  # Replicates plot(fit, which = 4): 
  #   - vertical lines ("type = 'h'") in base R
  #   - x-axis is observation index
  #   - y-axis is Cook's distance
  p4 <- residuals_vs_leverage_plot(model)
...
for (i in seq_along(set_ds)) {
  plots_list[[i]] <- my_diagnostic_plots(set_fit[[i]], set_ds[[i]])
}
...
fit_models <- list(mean = fit_mean, median = fit_median, list = fit_list, reg = fit_reg, rf = fit_rf, gam = fit_gam, or = fit_original)
...
# Plotting selected coefficients with specified y-axis limits and using the aurora palette
ggplot(selected_coef_df, aes(x = Term, y = Estimate, color = Model)) +
  ...
  scale_color_nord("aurora") +  # Use the aurora color palette
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
pred_origin <- predict(fit_original, newdata = test_data)

# Define a list of prediction methods and their corresponding names
predictions <- list(
  "Listwise deletion" = pred_list,
  "Mean Imputation" = pred_mean,
  "Median Imputation" = pred_median,
  "Linear regression (with noise)" = pred_reg,
  "Random Forest" = pred_rf,
  "GAM Imputation" = pred_gam,
  "Original Dataset" = pred_origin
)

# Calculate RMSE for each method and print the results
for (method in names(predictions)) {
  rmse_value <- rmse(predictions[[method]], test_data$mpg)
  cat(method, "(RMSE):", rmse_value, "\n")
}
```
